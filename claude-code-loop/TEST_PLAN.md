...
- UI state projection and hover details once the UI surface is implemented.
...
└── test/
    ...
    └── ui.test.tsx                 # Add only when src/ui exists
```
...
8. Disposing the agent removes all three tools and stops its timer.
...
| --- | --- |
| `loop_create({ prompt: 'check', time_in_seconds: 5 })` | Accepted; `allow_steer` is `true` |
...
3. `loop_delete` appends a versioned delete event and flushes before success.
4. Dispatch appends a versioned dispatch event with a strictly later
   `next_at` and flushes before the drive is considered complete.
5. A flush returning false rejects the operation and does not claim durability.
6. A flush rejection is propagated and does not produce a false success result.
7. Folding a new runtime from the existing event log restores active loops.
8. Deleted loops are absent after restart.
9. Dispatch state survives restart with the next occurrence intact.
10. Unrelated events do not affect loop state.
11. Two sessions do not share loop records, timers, or tool visibility.
12. A session fork follows the documented seed policy. For v1, active loops
...
13. A malformed persisted loop event fails closed with a useful error.
...
1. Concurrent `loop_create` calls produce independent records and ordered
   event appends.
2. Concurrent list/delete operations are serialized with create operations.
3. Deleting one loop cannot delete another loop with a similar ID.
4. A timer from session A never calls Agent B's `steer` or `followup`.
5. A tool invocation from session A cannot read session B's events.
6. A stale runtime reference cannot deliver after its agent is disposed.
7. Plugin disposal waits for an in-flight operation or safely cancels it;
...
1. No active loop renders no loop badge.
2. One active loop renders its prompt, interval, and scheduled/overdue state.
3. Multiple loops render without collapsing records from different IDs.
4. The UI distinguishes scheduled, overdue, and disabled/deleted state as
   defined by the projection contract.
5. Session switching replaces the projection; loop A is not visible in
   session B.
6. Hovering the loop indicator exposes the session-local loop ID and the
   session ID, if the UI contract includes both.
7. UI updates after a folded `loop/change` event without duplicating rows.
8. An empty or malformed projection fails safely and does not crash the chat
   view.
9. Rendering the UI does not start a timer or cause a persistence flush.
...
- all three schemas reject invalid input at the registry boundary;
