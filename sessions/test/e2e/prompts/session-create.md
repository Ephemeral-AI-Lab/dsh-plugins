...
exactly once. Do not substitute `create_session` or another tool.
...
Do not call `session_status`. After the tool call, report only
the returned `session_id`, `accepted`, and `status` fields.
