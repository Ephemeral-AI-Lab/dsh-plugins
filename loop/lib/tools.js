...
import { createLoopRecord, flushPersistence, foldLoopEvents, LoopInputError, loopView, } from './loop.js';
...
export function registerLoopTools(rootCtx, toolCtx, agent, runtime) {
    const disposers = [
        ...
        toolCtx.tools.register(defineTool({
            ...
            async execute(args, _exec) {
                if (typeof args.id !== 'string' || args.id.trim().length === 0)
                    throw new LoopInputError('id must be non-empty');
                return runtime.transact(async () => {
                    await flushPersistence(rootCtx, agent);
                    const folded = fold(agent.session);
                    if (!folded.active.some(loop => loop.id === args.id))
                        throw new LoopInputError(`unknown loop id: ${args.id}`);
