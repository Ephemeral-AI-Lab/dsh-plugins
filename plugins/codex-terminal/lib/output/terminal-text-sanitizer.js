...
export class TerminalTextSanitizer {
    pending = '';
    push(text) {
        if (text.length === 0 && this.pending.length === 0)
            return '';
...
function consumeEscapeSequence(text, start) {
    if (start + 1 >= text.length)
        return undefined;
    const kind = text[start + 1];
    if (kind === '[')
        return consumeCsi(text, start);
...
//# sourceMappingURL=terminal-text-sanitizer.js.map