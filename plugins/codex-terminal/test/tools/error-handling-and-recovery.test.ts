...
describe('error handling and recovery', () => {
  ...
  it('reports an unknown session as a tool error', async () => {
    const result = await callTool(harness.writeStdin, { job_id: 'codex-terminal-999', chars: '' }, execution(agent))
  ...
  it('reports writes to a completed session as a tool error', async () => {
    ...
    const result = await callTool(harness.writeStdin, { job_id: 'codex-terminal-1', chars: '' }, execution(agent))
