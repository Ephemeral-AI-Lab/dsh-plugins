...
export class DshJsonlConversionError extends MockScriptError {
    line;
    constructor(line, message, code = 'INVALID_SCRIPT', path) {
...
export function parseCanonicalScript(input, path) {
    let value;
    try {
...
export function validateMockScript(value, path) {
    const root = asPlainObject(value, 'script', path);
    assertExactKeys(root, ['type', 'version', 'steps'], 'script', path);
    if (root.type !== 'dsh-mock-script')
        invalid('type must be "dsh-mock-script"', path);
    if (root.version !== 1)
        invalid('unsupported script version; expected version 1', path);
    if (!Array.isArray(root.steps))
        invalid('steps must be an array', path);
    const steps = [];
    for (const [index, rawStep] of root.steps.entries()) {
        ...
        if (keys.length === 0)
            invalid(`${stepPath} must contain one step kind`, path);
        if (Object.prototype.hasOwnProperty.call(step, 'tool')) {
            ...
            if (!Array.isArray(members) || members.length === 0)
                invalid(`${stepPath}.parallel must be a non-empty array`, path);
            const parallel = [];
            for (const [memberIndex, rawMember] of members.entries()) {
            ...
            if (!Number.isSafeInteger(step.wait) || step.wait < 0) {
                invalid(`${stepPath}.wait must be a non-negative safe integer`, path);
            }
            steps.push({ wait: step.wait });
        }
        else {
...
export function compileMockScript(value, overwriteWaitTimeMs) {
...
export function serializeMockScript(value) {
 ...
 */
export function convertDshJsonl(input, path) {
    ...
    for (const [index, sourceLine] of input.split(/\r?\n/).entries()) {
        ...
        if (!sawSession) {
            if (record.type !== 'session')
                throw conversion(line, 'first nonblank record must have type "session"', path);
            sawSession = true;
            continue;
        }
        if (record.type === 'session')
            throw conversion(line, 'only one session header is allowed', path);
    ...
    if (!seenRecord || !sawSession)
        throw conversion(1, 'session record is missing', path);
    ...
    if (steps.length === 0)
        throw conversion(1, 'DSH JSONL contains no executable tool call', path);
    return validateMockScript({ type: 'dsh-mock-script', version: 1, steps }, path);
}
function validateToolStep(value, stepPath, path) {
    if (typeof value.tool !== 'string' || !isMockToolName(value.tool))
        invalid(`${stepPath}.tool is not a valid tool name`, path);
    if (!isPlainObject(value.args))
        invalid(`${stepPath}.args must be a JSON object`, path);
    const args = cloneJsonObject(value.args, `${stepPath}.args`, path);
    return { tool: value.tool, args };
}
function reconcileGroup(group, path) {
    const completeCalls = [
        ...group.messageCalls,
        ...group.completeCalls,
        ...group.durableCalls,
    ...
    if (complete.length > 0 && chunkCalls.length > 0)
        compareCallLists(preferred, chunkCalls, path);
    return (complete.length > 0 ? preferred : chunkCalls).sort((left, right) => left.order - right.order);
}
function uniqueCalls(candidates, path) {
    const byId = new Map();
...
function compareCallLists(complete, chunks, path) {
    if (complete.length !== chunks.length) {
        throw conversionMismatch(chunks.at(-1)?.line ?? complete.at(-1)?.line ?? 1, `complete assistant records contain ${complete.length} call(s), but chunk records contain ${chunks.length}`, path, complete.at(-1)?.line, chunks.at(-1)?.line);
    }
...
function groupFor(groups, record, line) {
...
function addRecordTime(group, record, line) {
...
function collectMessageCalls(group, record, line, path) {
    ...
    if (!Array.isArray(message.content))
        throw conversion(line, 'data.message.content must be an array', path);
...
function collectAssistantChunk(group, record, line, path) {
    const data = requiredObject(record.data, line, 'data', path);
    ...
    if (chunk.type === 'block-end') {
        ...
        if (block.type === 'tool-call')
            group.completeCalls.push(candidateFromBlock(block, line, 0, 'assistant/chunk', path));
        return;
    }
    ...
    if (chunk.name !== undefined) {
        if (typeof chunk.name !== 'string')
            throw conversion(line, 'tool-call-delta.name must be a string', path);
    ...
    if (typeof fragment !== 'string')
        throw conversion(line, 'tool-call-delta.argumentsDelta must be a string', path);
    appendFragment(partial, fragment, line, recordOrder(record, line), 0);
    group.deltaCalls.set(id, partial);
}
function collectPackedCall(group, record, line, path) {
    const data = requiredObject(record.data, line, 'data', path);
    ...
    if (Array.isArray(data.args)) {
        for (const [index, fragment] of data.args.entries()) {
            if (typeof fragment !== 'string')
                throw conversion(line, 'tool-call-chunks.args must contain strings', path);
...
function collectDurableCall(group, record, line, path) {
    const data = requiredObject(record.data, line, 'data', path);
...
function candidateFromBlock(block, line, index, representation, path) {
...
function chunkCandidates(primary, supplement, path) {
    const ids = new Set([...primary.keys(), ...supplement.keys()]);
    return [...ids]
        .map(id => [primary.get(id), supplement.get(id)].filter((part) => part !== undefined))
...
function completeChunkArguments(parts, path) {
    const nonEmpty = parts.filter(part => part.rawArguments.length > 0);
    if (nonEmpty.length === 0)
        throw incompleteChunk(parts, 'chunk call has no argument fragments', path);
...
function incompleteChunk(parts, message, path) {
    const first = parts[0];
...
function createPartialCall(id, order, line, representation) {
    return { id, order, rawArguments: '', line, lastLine: line, recordLines: [line], fragments: [], representation };
}
function appendFragment(partial, text, line, order, index) {
    partial.rawArguments += text;
    partial.fragments.push({ text, line, order, index });
}
function touchPartial(partial, line) {
    partial.lastLine = line;
    if (partial.recordLines.at(-1) !== line)
        partial.recordLines.push(line);
}
...
function mergeChunkText(first, second) {
    if (first.length === 0)
        return second;
    if (second.length === 0)
        return first;
...
function normalizeCall(name, rawArguments, line, path) {
    if (!isMockToolName(name))
        throw conversion(line, `invalid tool name ${JSON.stringify(name)}`, path);
    let args;
    try {
    ...
    if (!isPlainObject(args))
        throw conversion(line, 'tool arguments must be a JSON object', path);
    return { tool: name, args: cloneJsonObject(args, 'tool arguments', path) };
}
function parseRecord(text, line, path) {
    try {
        const value = JSON.parse(text);
        if (!isPlainObject(value))
            throw conversion(line, 'record must be a JSON object', path);
        return value;
    }
    catch (error) {
        if (error instanceof DshJsonlConversionError)
            throw error;
        throw conversion(line, 'record must be valid JSON', path);
    }
}
function recordOrder(record, line) {
...
function requiredObject(value, line, label, path) {
    if (!isPlainObject(value))
        throw conversion(line, `${label} must be an object`, path);
    return value;
}
function requiredString(value, line, label, path) {
    if (typeof value !== 'string' || value.length === 0)
        throw conversion(line, `${label} must be a non-empty string`, path);
    return value;
}
...
function cloneJsonObject(value, label, path) {
    const output = {};
    for (const [key, child] of Object.entries(value))
        output[key] = cloneJsonValue(child, `${label}.${key}`, path);
    return output;
}
function cloneJsonValue(value, label, path) {
    ...
    if (Array.isArray(value))
        return value.map((child, index) => cloneJsonValue(child, `${label}[${index}]`, path));
    if (isPlainObject(value))
        return cloneJsonObject(value, label, path);
    invalid(`${label} must contain JSON values`, path);
}
function assertExactKeys(value, expected, label, path) {
...
function asPlainObject(value, label, path) {
    if (!isPlainObject(value))
        invalid(`${label} must be a JSON object`, path);
    return value;
}
...
function conversion(line, message, path) {
    return new DshJsonlConversionError(line, message, 'INVALID_SCRIPT', path);
}
function conversionMismatch(line, message, path, firstLine, secondLine) {
