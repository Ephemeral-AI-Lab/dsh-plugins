...
export function isMockToolName(value) {
...
export function parseMockCommand(input) {
    ...
    if (body.startsWith('run'))
        return parseRunCommand(body);
    if (body.startsWith('replay'))
        return parseReplayCommand(body);
    throw new MockCommandParseError('expected `run` or `replay` after /mock');
}
...
export function parseRunCommand(input) {
    const match = /^run(?=[\t\n\r ]|$)/.exec(input);
    if (match === null)
        throw new MockCommandParseError('expected /mock run');
    ...
    if (input[cursor] === '[') {
        ...
        while (true) {
            cursor = skipWhitespace(input, cursor);
            if (cursor >= input.length)
                throw new MockCommandParseError('unterminated parallel group');
            if (input[cursor] === ']') {
                if (calls.length === 0)
                    throw new MockCommandParseError('parallel group must not be empty');
                cursor += 1;
                break;
            }
            const parsed = parseExpressionAt(input, cursor);
            if (parsed.call.name === 'wait')
                throw new MockCommandParseError('wait syntax is only valid in canonical replay scripts');
...
export function parseReplayCommand(input) {
    const match = /^replay(?=[\t\n\r ]|$)/.exec(input);
    if (match === null)
        throw new MockCommandParseError('expected /mock replay');
    const tokens = tokenizeReplayArguments(input.slice(match[0].length));
    if (tokens.length === 0)
        throw new MockCommandParseError('replay requires a source path');
    const path = tokens[0];
    if (path === undefined || path.length === 0)
        throw new MockCommandParseError('replay requires a source path');
    if (tokens.length === 1)
        return { kind: 'replay', path };
...
function parseExpressionAt(input, start) {
    ...
    cursor = skipWhitespace(input, cursor);
    if (input[cursor] !== '(')
        throw new MockCommandParseError('expected `(` after tool name');
    const open = cursor;
    cursor += 1;
    const close = findClosingParenthesis(input, cursor);
    if (close < 0)
        throw new MockCommandParseError('unterminated tool call');
...
function findClosingParenthesis(input, start) {
    let inString = false;
    let escaped = false;
...
function tokenizeReplayArguments(input) {
    const tokens = [];
    let cursor = 0;
        ...
        if (quote === '"') {
            ...
            while (cursor < input.length) {
                ...
                if (character === '\\') {
                    ...
                    if (next === undefined)
                        throw new MockCommandParseError('unterminated quoted replay path');
