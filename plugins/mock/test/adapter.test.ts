...
import { validateMockScript } from '../src/converter.js'
...
describe('MockAdapter', () => {
  ...
  it('does not let auxiliary model calls consume a pending mock plan', async () => {
    const fallbackStream = (): AsyncIterable<StreamChunk> => (async function* () {
      yield { type: 'text-delta', index: 0, text: 'auxiliary response' }
      yield { type: 'finish', reason: { kind: 'stop' } }
    })()
    const adapter = new MockAdapter(undefined, () => fallbackStream())
    ...
    expect(next.some(chunk => chunk.type === 'text-delta' && chunk.text.includes('Mock run completed (1 executable step).'))).toBe(true)
  ...
  it('reports malformed input as text and does not emit a tool call', async () => {
    ...
    expect(chunks.some(chunk => chunk.type === 'text-delta' && chunk.text.includes('Invalid mock command'))).toBe(true)
  ...
  it('keeps call state isolated by session and releases it after each result', async () => {
    ...
    expect(firstSummary.some(chunk => chunk.type === 'text-delta' && chunk.text.includes('Mock run completed (1 executable step).'))).toBe(true)
    expect(secondSummary.some(chunk => chunk.type === 'text-delta' && chunk.text.includes('Mock run completed (1 executable step).'))).toBe(true)
  ...
  it('returns an aborted finish for a cancelled request', async () => {
    ...
    expect(chunks).toEqual([{
      type: 'finish',
      reason: { kind: 'aborted', failure: { message: 'mock request aborted', code: 'ABORTED' } },
    }])
    expect(adapter.pendingSessionCount).toBe(0)
  })
  ...
  it('keeps a replacement plan alive when a retired result arrives late', async () => {
    ...
    adapter.startPlan({
      ...
      runId: 'run-old',
      mode: 'run',
      script: validateMockScript({ type: 'dsh-mock-script', version: 1, steps: [{ tool: 'tool_a', args: {} }] }),
    })
    ...
    adapter.startPlan({
      ...
      runId: 'run-new',
      mode: 'run',
      script: validateMockScript({ type: 'dsh-mock-script', version: 1, steps: [{ tool: 'tool_b', args: {} }] }),
    })
  ...
  it('terminates a cross-session result handoff with MOCK_PROTOCOL', async () => {
    const adapter = new MockAdapter()
    adapter.startPlan({
      sessionId: 'owner',
      runId: 'run-owner',
    ...
    expect(terminal.at(-1)).toMatchObject({
      type: 'finish',
      reason: { kind: 'error', failure: { code: 'MOCK_PROTOCOL' } },
    })
    expect(adapter.pendingSessionCount).toBe(0)
  })
  ...
  it('clears pending state on adapter disposal', async () => {
    ...
    expect(toolCall(chunks).id).toMatch(/^mock-/)
    expect(adapter.pendingSessionCount).toBe(1)
    adapter.dispose()
    expect(adapter.pendingSessionCount).toBe(0)
  })
