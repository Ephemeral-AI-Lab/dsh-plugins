...
describe('native DSH JSONL converter', () => {
  ...
  it('reassembles interleaved packed and assistant chunks from the external fixture shape', () => {
    ...
    const input = [
      ...
      record({
        type: 'assistant/chunk',
        data: { turn: 3, step: 4, chunk: { type: 'tool-call-delta', id: 'call-1', name: 'exec_command', argumentsDelta: 'ugins/mock' } },
      }),
      record({
        type: 'assistant/chunk',
        data: { turn: 3, step: 4, chunk: { type: 'tool-call-delta', id: 'call-1', name: 'exec_command', argumentsDelta: ' && node nod' } },
      }),
      record({
        type: 'tool-call-chunks',
        data: {
