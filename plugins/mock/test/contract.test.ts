...
describe('mock command and scheduling contract', () => {
  it('rejects direct ZIP replay with an explicit extraction diagnostic', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'dsh-mock-archive-'))
  ...
  it('accepts only one run unit and compiles executable progress correctly', () => {
    expect(parseMockCommand('/mock run a({"x":[1,true,null]})')).toEqual({
      kind: 'run',
      calls: [{ name: 'a', arguments: { x: [1, true, null] } }],
    })
    expect(parseMockCommand('/mock run [a({}) b({"v":2})]').calls).toHaveLength(2)
    expect(() => parseMockCommand('/mock run a({}); b({})')).toThrow()
    expect(() => parseMockCommand('/mock run wait(250)')).toThrow()
    expect(() => parseMockCommand('/mock run a({}) --wait 10')).toThrow()
    ...
    expect(compileMockScript(script).steps.map(step => step.waitBefore)).toEqual([0, 250, 100])
    expect(compileMockScript(script, 50).steps.map(step => step.waitBefore)).toEqual([0, 50, 100])
    expect(() => validateMockScript({ type: 'dsh-mock-script', version: 1, steps: [{ wait: 1 }, { tool: 'a', args: {} }] })).toThrow()
    expect(() => validateMockScript({ type: 'dsh-mock-script', version: 1, steps: [{ tool: 'a', args: {} }, { wait: 1 }, { wait: 2 }, { tool: 'b', args: {} }] })).toThrow()
  })

  it('routes slash mock through the real ToolRuntime, then restores the real provider', async () => {
    ...
    try {
      const mockIdle = waitForIdle(ctx, agent)
      const command = await ctx.commands.execute(
        agent,
        '/mock run probe_tool({"value":"mock"})',
        new AbortController().signal,
      )
      expect(command?.result).toEqual({ kind: 'success', text: 'Mock command queued.' })
      await mockIdle
      expect(executed).toEqual(['mock'])
      expect(agent.options.provider).toBe('recording-real')
      expect(agent.session.events.some(event => event.type === 'assistant/message' && event.data.message.source.provider === 'mock')).toBe(true)
      const mockFinal = agent.session.events.filter(event => event.type === 'assistant/message').at(-1)
      expect(mockFinal?.data.message.content.some(block => block.type === 'text'
        && block.text.includes('Mock run completed (1 executable step).'))).toBe(true)
      expect(mockFinal?.data.message.content.some(block => block.type === 'text'
        && block.text.includes('RESULT:mock'))).toBe(false)
      ...
      expect(real.requests.at(-1)).not.toContain('mock')
  ...
  it('cancels a real tool execution, clears replay state, and restores the real route', async () => {
    ...
    try {
      ...
      send(agent, '/mock run slow_probe({})')
      ...
      expect(agent.session.events.findLast(event => event.type === 'mock/status')?.data).toMatchObject({
        phase: 'cancelled',
        errorCode: 'CANCELLED',
      })
  ...
  it('replays canonical JSON in memory with an implicit gap and leaves the source unchanged', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'dsh-mock-'))
    ...
    const agent = ctx.agentLoop.create(SessionId('replay-contract'), { provider: 'mock', model: 'mock' })
    try {
      const idle = waitForIdle(ctx, agent)
      send(agent, `/mock replay "${sourcePath}" --overwrite-wait-time-ms 0`)
      await idle
