# Mock UI/UX design and logic

This document defines the small UI/UX surface for the external `mock`
plugin. Mockging is initiated by a slash command in the existing DSH session;
...
- Never invent a tool call, tool result, or parallel-execution message in the
  UI.
- Keep mock state scoped to `sessionId` and `runId`, so another session cannot
...
the composer and disappear when there is no active mock run.

Do not add a panel to the model page, a sidebar, or a large mock transcript.
The submitted `/mock ...` text may remain the normal user message, but the
...
![DSH mock UI/UX wireframe](./dsh-mock-ui-wireframe.svg)

The blue row in the illustration is the proposed mock component. It sits
...
```mermaid
flowchart LR
  MockSurface["Mock UX surface"] --> StatusRow["MockStatusRow"]
  StatusRow --> Icon["StatusIcon"]
  StatusRow --> Label["StatusLabel"]
  StatusRow --> Progress["ProgressText"]
  MockSurface --> ErrorCard["MockErrorCard\ninvalid script / stopped run"]
  MockSurface --> LiveRegion["Accessible live region\nterminal status"]
  State["MockUiState\n(sessionId, runId)"] --> MockSurface
  DSH["agent/status + normal DSH events"] --> State
```

`MockStatusRow` is the only persistent active component. `MockErrorCard` is
...
The status is `Mock replay · 1/3`, then `2/3`, then `3/3`. While the
...
For `/mock run`, `totalSteps` is `1` for a single tool or one parallel group.

## 5. UX behavior by scenario

| Scenario | Component state | User-visible behavior | Runtime meaning |
| --- | --- | --- | --- |
| No mock run | No active row | Normal composer only | No mock state exists. |
| Valid `/mock run` | `queued` then `running` | Show a generic spinner and `Mock run - 1/1` | The real AgentLoop turn is active. |
| Valid `/mock replay` | `queued` then `running` | Show `Mock replay - current/total` | The in-memory canonical queue is being consumed. |
...
| Invalid command/script | `failed` | Show `Mock script invalid - INVALID_SCRIPT` with source location; no active spinner remains | Validation stopped the run before the adapter streamed. |
...
| Mock follow-up or steer | New `queued` mock turn | Route the slash command to mock; follow-up and steer look the same | Both are treated as the next mock turn. |
| Non-slash follow-up or steer | No mock row unless another run exists | Use the normal real-agent UI | The configured real provider handles the turn. |
| Background-job tool | `running` until parent tool returns | Keep mock progress tied to the parent step; show job status through normal DSH job UI | Background work is not flattened into replay steps. |
...
1. Classify a message beginning with `/mock` as a mock turn. A mock command
   received through follow-up or steer uses the same next-turn queueing
   semantics; steer has no additional meaning for mock.
...
3. Create an in-memory `MockUiState` and a session-scoped replay queue.
...
`mock/run-started`, `mock/step-started`, `mock/wait-started`,
`mock/step-completed`, `mock/run-failed`, `mock/run-completed`, and
`mock/run-cancelled`. These are UI hints, not replacements for DSH's durable
...
sequenceDiagram
  ...
  participant Router as Mock command router
  participant State as MockUiState
...
```text
Mock script invalid · INVALID_SCRIPT
...
If the next message starts with `/mock`, route it to mock command handling,
...
normal real provider path. Therefore one session can alternate between mock
turns and real-agent turns without changing the configured model page.

## 9. Background tools and subagents

A tool that starts a background job is allowed. The mock row tracks the
top-level tool step until the tool returns; the normal DSH job UI/lifecycle
tracks the background job separately. The mock row must not claim that the
