...
function jwtExpiry(token: string): number {
  ...
  if (payload === undefined) throw new Error('codex-coding-plan: Codex access token is not a JWT')
  let decoded: unknown
  try {
...
export function parseCodexAuth(raw: string): LoadedCodexAuth {
  let decoded: unknown
  try {
  ...
  if (tokens === undefined) throw new Error('codex-coding-plan: Codex auth cache has no tokens')
  ...
  return {
    ...
    document: decoded as CodexAuthDocument,
  }
}

export function readCodexAuth(authPath: string): Promise<LoadedCodexAuth | undefined> {
  return readOAuthFile(authPath, parseCodexAuth, 'codex-coding-plan: Codex')
}

export function currentCodexAuth(
  authPath: string,
  refresh: RefreshOAuth,
  now = Date.now(),
): Promise<LoadedCodexAuth | undefined> {
  return currentOAuthFile(
    authPath,
    parseCodexAuth,
    (document, refreshed, timestamp): CodexAuthDocument => {
