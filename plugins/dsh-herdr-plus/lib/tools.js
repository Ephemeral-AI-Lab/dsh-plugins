...
const AGENT_STATES = ['idle', 'working', 'blocked', 'done', 'unknown'];
 ...
 */
async function resolvePane(deps, target, signal) {
    const got = await runHerdrJson(deps.env, ['agent', 'get', target], { signal });
    if (got.ok) {
        const agent = agentRecord(got.result);
        if (agent !== undefined && typeof agent.pane_id === 'string')
            return { paneId: agent.pane_id, agent };
    }
    ...
    if (PANE_ID_RE.test(target)) {
        ...
        if (!pane.ok)
            throw asError(pane, `pane get ${target}`);
 ...
 */
async function pollUntil(probe, timeoutMs, signal) {
    const deadline = Date.now() + timeoutMs;
    for (;;) {
        const value = await probe();
        if (value !== undefined || Date.now() >= deadline)
            return value;
        if (!(await delay(POLL_INTERVAL_MS, signal)))
            return undefined;
    }
}
/**
 ...
 */
export function registerHerdrTools(tools, env, options) {
    const deps = {
        env,
    ...
    const spawn = tools.register(defineTool({
        ...
        description: 'Split a sibling pane in the current Herdr tab and launch a new dsh session in it ' +
        ...
        async execute(args, exec) {
            ...
            if (!split.ok)
                throw asError(split, 'pane split');
            const pane = split.result.pane;
            const paneId = pane?.pane_id;
            if (typeof paneId !== 'string')
                throw new Error('pane split reply carried no pane_id');
    ...
    const prompt = tools.register(defineTool({
        ...
        description: 'Send a follow-up prompt to a sibling agent: the text is typed into its pane and ' +
        ...
        async execute(args, exec) {
            ...
            if (agent?.agent_status === 'blocked') {
                throw new Error(`agent "${args.target}" is blocked on its own approval or question; ` +
    ...
    const wait = tools.register(defineTool({
        ...
        description: 'Wait until a sibling agent reaches a requested state. Defaults match Herdr: ' +
        ...
        async execute(args, exec) {
            ...
            if (!waited.ok) {
                if (waited.code === 'timeout')
                    return { status: 'timeout', matched: false };
    ...
    const read = tools.register(defineTool({
        ...
        description: 'Read a sibling agent\'s pane output. `visible` shows its current screen; ' +
        ...
        async execute(args, exec) {
            ...
            if (!read.ok)
                throw asError(read, `pane read ${paneId}`);
...
function waitArgs(target, until, timeoutMs) {
    const args = ['agent', 'wait', target];
    for (const state of until ?? [])
        args.push('--until', state);
    args.push('--timeout', String(timeoutMs ?? 120_000));
    return args;
}
async function waitFor(deps, target, until, timeoutMs, signal) {
    const waited = await runHerdrJson(deps.env, waitArgs(target, until, timeoutMs), { signal });
    if (!waited.ok)
        return waited.code === 'timeout' ? 'timeout' : `error:${waited.code}`;
    const agent = agentRecord(waited.result);
