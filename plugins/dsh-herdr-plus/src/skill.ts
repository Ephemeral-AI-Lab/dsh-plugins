...
function candidate(skillsUrl: URL): SkillCandidate {
  return {
    name: 'herdr',
    description:
      'Control Herdr workspaces, tabs, panes, and agents from inside a Herdr pane, ' +
...
async function loadBody(options: HerdrSkillOptions, signal?: AbortSignal): Promise<string | undefined> {
  ...
  if (options.mode === 'auto') {
    ...
    if (out.ok && out.text.includes('HERDR_ENV')) return out.text
  }
  try {
...
async function loadAddendum(options: HerdrSkillOptions, signal?: AbortSignal): Promise<string> {
  try {
    const skillsUrl = options.skillsUrl ?? SKILLS_URL
 ...
 */
export function makeHerdrSkillProvider(options: HerdrSkillOptions): SkillProvider {
  const entry = candidate(options.skillsUrl ?? SKILLS_URL)
  return {
    name: 'dsh-herdr-plus',
    list: () => Promise.resolve([entry]),
    async get(_candidate, lookup): Promise<SkillDefinition | undefined> {
