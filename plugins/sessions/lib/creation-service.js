...
export class SessionCreationService {
    ...
    async createSession(args, parent, signal) {
        ...
        const modelRef = { current: selection, assembled: undefined };
    ...
    async resolveSelection(explicit, parent, signal) {
    ...
    async resolvePreset(explicit, parent) {
        if (explicit !== undefined) {
            requireText(explicit, 'preset');
            const presets = this.ctx.get('agentPresets');
            if (presets === undefined)
                throw new Error('session_create.preset requires agent presets to be configured');
    ...
    async resolveLocation(args, parent) {
...
function createSetup(preset, modelRef) {
    if (preset === undefined && modelRef === undefined)
        return undefined;
...
function inheritedSelection(parent) {
...
function sessionMetadata(preset, location) {
    return {
...
async function optionalWorkspace(registry, cwd) {
    try {
