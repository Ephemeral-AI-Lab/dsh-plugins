...
For every scenario, report the exact tool calls, returned `session_id` values,
output, exit codes, and any unexpected behavior. If a call returns a
`session_id`, continue with `write_stdin` until the session reaches its
...
2. Verify the result contains exactly `SMOKE-OK`, has exit_code 0, and does not leave an unexplained session.
3. If a session_id is returned, poll it once with write_stdin using chars="" and verify the final result is clean.
...
3. If a session_id is returned, use write_stdin with chars="" until the terminal result is available.
...
```text
Verify that a pipe-backed session accepts stdin through write_stdin.
...
2. If exec_command returns a session_id, call write_stdin with chars="hello-from-stdin\n".
3. Verify the output contains exactly `ECHO:hello-from-stdin`, the final exit_code is 0, and the session can be collected normally.
...
2. Wait briefly without replacing the session with another command.
3. Poll the returned session_id with write_stdin using chars="".
...
2. If the initial result returns a session_id, wait for natural completion and poll it with write_stdin(chars="").
3. Verify the final poll exposes `NATURAL-EXIT` and exit_code 0.
4. If an owner notification is surfaced, verify it tells the agent to call write_stdin with the same session_id and empty chars.
5. Poll once more only if the contract indicates unread output remains; otherwise verify the session is no longer available after the terminal result.
...
2. Once the session_id is available, call write_stdin with chars="" and max_output_tokens=100.
3. Treat a capped result as an intermediate page even if it already contains exit_code 0. Continue polling the same session with empty chars and the same output limit until session_id disappears.
4. Concatenate all returned output and assert every line LINE-0 through LINE-159 appears exactly once, in order.
5. Assert the final result has exit_code 0 and that a further poll reports the documented unknown/completed-session behavior.
...
Keep the same session_id for all calls. Record whether each call returned promptly after new output or process exit.
```
...
2. Poll the four session_ids in round-robin order with write_stdin(chars=""), allowing each session to finish independently.
3. Verify each job's output and exit code: A/B/D exit 0, C exits 3 with its stderr visible.
4. Do not poll one session through another session_id, and do not assume completion order matches start order.
5. After all terminal results are collected, verify no session remains pollable and report any cleanup or capacity anomaly.
