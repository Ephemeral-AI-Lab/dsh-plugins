...
describe('automatic background-job promotion', () => {
  ...
  it('promotes a command that outlives yield_time_ms and names both ids', async () => {
    ...
    expect(started.value).toMatchObject({ job_id: 'codex-shell-1' })
    expect(started.content[0]?.text).toContain('[job_id: codex-shell-1]')
  ...
  it('settles the tracked job, steers its owner, and keeps terminal polls idempotent', async () => {
    ...
    expect(steer).toHaveBeenCalledWith(expect.objectContaining({
      content: [{
        type: 'text',
        text: 'exec job codex-shell-1 exited with code 0. Call write_stdin with job_id="codex-shell-1" and chars="" to collect the remaining output.',
      }],
    }))
    ...
    expect(first.content[0]?.text).toContain('[job codex-shell-1 exited with code 0]')
    ...
    expect(repeated.content[0]?.text).toBe('[job codex-shell-1 exited with code 0; no unread output remains]')
  })
