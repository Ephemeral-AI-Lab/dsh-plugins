# DSH debug-agent exhaustive test specification

Status: test-plan authoring document for the multi-step debug/replay contract.
...
must continue to use the normal DSH AgentLoop and ToolRuntime. The debug
...
1. A valid debug command or replay has one canonical representation before it
...
The command router must recognize only the debug slash-command forms. The
examples below are normative fixtures for the selected public spelling:

~~~text
/debug run probe_tool({"value":"ok"})
/debug run [probe_tool({"value":"a"}) probe_tool({"value":"b"})]
/debug replay scripts\one.json
/debug replay "C:\test fixtures\one.jsonl" --overwrite-wait-time-ms 250
~~~
...
| CMD-16 | /debug, /debug run, or whitespace-only command | Reject with usage diagnostic |
| CMD-17 | /debug run containing a wait object, wait(...), --wait, sleep, or a timing flag | Reject: run has no wait syntax |
...
/debug, wait, or shell syntax. Only the command tokens and JSON structure
define behavior.

## 4. /debug replay path and option parsing
...
| --- | --- | --- |
| SCH-01 | Validate a self-authored /debug run script | Validator accepts and returns the canonical representation |
...
| SCH-15 | Missing/wrong type, wrong case, or unsupported version | Reject; only dsh-debug-script and version 1 |
...
AgentLoop, and the compiled or source debug plugin. Register the fixtures with
...
| RT-24 | Real provider route configured in another agent | Debug plugin does not intercept or mutate it |
...
| ID | Operation | Expected assertion |
| --- | --- | --- |
| SES-01 | Send /debug run ... as a followup while a real turn is active | Debug command is queued as the next turn; current real turn is not rewritten |
...
| SES-03 | Compare followup and steer for the same debug command | Equivalent events, tool args, waits, completion, and errors |
| SES-04 | Interrupt an active debug stream | Current debug turn aborts; queued later step is not silently executed |
| SES-05 | Interrupt an explicit wait | Wait timer cancels; state returns terminal/idle according to host contract |
| SES-06 | Non-slash followup text | Routes to the configured normal/real provider; no debug parser invocation |
| SES-07 | Non-slash steer text | Routes normally; no debug queue or debug provider substitution |
| SES-08 | Normal real-provider turn followed by debug turn | Both turns coexist in one session with separate provider/stream semantics |
| SES-09 | Debug turn followed by normal real-provider turn | Debug pending state is consumed/cleared; real turn sees correct history |
...
A slash command is debug input by its command classification, whether it
arrives through followup or steer. A non-slash message is never reinterpreted
as debug merely because it resembles a tool expression.

## 12. Background jobs, nested agents, and child sessions

### 12.1 Background jobs

background_start is a real registered tool, so the debug plugin must not block
...
| SES-16 | Same adapter instance serves parallel debug sessions | No global nextCall/cursor state |
...
| --- | --- | --- |
| UI-01 | Debug script starts | Agent is running and command is visible |
...
Wait duration can be exposed by a dedicated debug wait indicator, but it must
...
required for debug paths.

| ID | Journey | Expected assertion |
| --- | --- | --- |
| E2E-01 | Start profile with compiled debug-agent plugin | Profile boots; mock-debug/debug model is discoverable; no host source change |
| E2E-02 | Enter valid single /debug run | One tool executes, result renders, session becomes idle/completed |
...
run the host's existing E2E/web suites with the debug profile/fixture wired by
the test harness:
...
- provider route is exactly mock-debug and model is debug;
...
The debug-agent test plan is satisfied only when every matrix row has a test
...
9. cleanup report showing zero pending sessions, timers, listeners, and
   registered debug provider after disposal.
...
debug/replay flow. These are historical incidents as well as regression cases;
future changes must preserve the stated behavior.

| ID | Observed symptom | Root cause | Required regression assertion |
|---|---|---|---|
| HIS-01 | The deterministic debug model appeared as an explicit model choice. | The debug provider/model route leaked into the normal model-selection surface. | `/debug` is the only debug entry point; no deterministic debug model is shown as a selectable model. |
| HIS-02 | `/debug` was absent or its browser UI was not loaded. | The patch referenced a local `file:` entry, so the web plugin scanner could not resolve package metadata or the client half. | The patch resolves the package by name; the package exports `./client`; the boot manifest contains `dsh-debug-agent/client.js`; `/debug` is registered. |
| HIS-03 | It was unclear whether slash commands used a fake executor or the real agent. | The intended boundary was not explicit. | The slash command queues a normal user message; AgentLoop and ToolRuntime perform routing, validation, authorization, execution, and durable call/result events; the debug adapter emits only model chunks. |
| HIS-04 | A “System Prompt Updated” row appeared even though prompt mutation was not wanted. | A plugin request/header hook modified the system prompt; the host also naturally logs request/header and context-injection snapshots. | The debug plugin does not register a `request/header` mutation or update the system prompt. Host-owned context snapshots may remain ordinary logs, but no plugin-owned prompt mutation is emitted. |
| HIS-05 | Debug runs showed thinking/reasoning behavior. | The real route’s `reasoningEffort` was passed to the deterministic debug route. | The debug request omits `reasoningEffort`; no reasoning/thinking block is requested from `mock-debug`. |
...
| HIS-12 | After removing the timeout, the replay could remain blocked even after the tool finished. | Auxiliary `session-title`/compaction LLM calls entered the same debug plan and competed for its pending result/cursor. | Auxiliary calls bypass the debug plan and use the real provider when available; only the actual AgentLoop request consumes replay steps. |
...
| HIS-16 | Local package-name resolution failed during profile testing. | The test profile did not have an installed `dsh-debug-agent` package, so package-name loading needed a local profile junction. | The test profile setup documents package resolution; normal installation resolves the package without modifying the host source checkout. |
| HIS-17 | A replay could appear to be waiting while a tool row already showed execution time. | Tool completion and the next AgentLoop request are separate event boundaries; the UI did not distinguish host execution from adapter handoff. | Status remains `waiting` only until the result reaches the debug adapter, then advances to the next step or terminal completion; tool duration alone is not treated as adapter completion. |
| HIS-18 | Debug routing could leak into the next ordinary user turn. | The temporary debug provider route was persisted or not restored after the debug turn. | After debug completion/failure/abort, the next normal turn uses the configured real provider/model; no debug model/header mutation remains. |
...
`Debug replay completed (7 executable steps)` after the live tool results
arrived.
...
- the adapter regression test covers auxiliary calls while a debug tool result
  is pending;
