...
construction. These cases exercise the external `debug-agent` contract from
...
| Code | Invariant |
| --- | --- |
| I1 | Every accepted input has one canonical `dsh-debug-script` representation before queue compilation. |
...
| --- | --- | --- | --- | --- | --- |
| C-01 | Easy | U | `session(S)` followed by one `message(1,1,[call("probe_tool", {"value":"ok"}, old-1)])`. | Produces canonical `{"type":"dsh-debug-script","version":1,"steps":[{"tool":"probe_tool","args":{"value":"ok"}}]}` with no source metadata in `steps`. | I1, I4 |
