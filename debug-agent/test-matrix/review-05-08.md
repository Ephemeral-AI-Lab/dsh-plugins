...
| Durable events | `RTI-15` plus the event assertions in `RTI-01`, `RTI-03..08`, `JOB-02`, `JOB-08` | Reload/no-replay is justified. Required debug lifecycle-event fields and the “optional extension unavailable” fallback are not asserted directly. |
...
| `RTE-17` | High | integration | After a debug runtime error and after a debug cancellation, send ordinary text. Both paths restore the configured real provider/model and leave the model-page selection unchanged. | 0.5–0.75 day / 1–2 s |
...
| `JOB-18` | Critical | unit | A result from an old debug turn in the same session arrives after a new turn begins. It must be diagnosed and must not advance the new turn, even if the call id shape is otherwise valid. | 0.5 day / <1 s |
| `EVT-01` | Medium | integration | Validate serializable debug lifecycle events and reload them: session id, debug turn id, source mode/format/path when allowed, script id, executable-step count, and terminal status; no duplicate tool events. If the host has no public extension point, assert normal DSH events plus the documented unavailable diagnostic. | 0.5–0.75 day / 1–2 s |
