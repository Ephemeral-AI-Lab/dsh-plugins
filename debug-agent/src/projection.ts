...
const projectionSchema = z.union([debugStateSchema, z.null()])

interface DebugStatusProjectionState {
  readonly value: DebugUiState | null
  readonly runId: string | null
  readonly seenRunIds: readonly string[]
}

/** Fold the latest log-only debug status into the client projection stream. */
export const debugStatusProjectionDefinition:
ProjectionDefinition<'debugStatus', DebugStatusProjectionState> = {
  key: 'debugStatus',
  schema: projectionSchema as unknown as ProjectionDefinition<'debugStatus', DebugUiState | null>['schema'],
  init: () => ({ value: null, runId: null, seenRunIds: [] }),
  apply: (state, event: SessionEvent) => {
    if (event.type !== 'debug/status') return state
