import { isDebugToolName } from './parser.js'
...
export interface DebugWaitStep {
  readonly wait: number
}

/** Backwards-compatible name retained for consumers of the initial scaffold. */
export type DebugDelayStep = DebugWaitStep

export interface DebugParallelStep {
  readonly parallel: readonly DebugToolStep[]
}

export type DebugScriptStep = DebugToolStep | DebugWaitStep | DebugParallelStep

export interface DebugScript {
  readonly type: 'dsh-debug-script'
  readonly version: 1
  readonly steps: readonly DebugScriptStep[]
}

export interface CompiledDebugStep {
  readonly sourceIndex: number
  readonly executableIndex: number
  readonly step: DebugToolStep | DebugParallelStep
  readonly waitBefore: number
  readonly explicitWaitBefore: boolean
}

export interface CompiledDebugScript {
  readonly script: DebugScript
  readonly steps: readonly CompiledDebugStep[]
  readonly executableStepCount: number
}

export type DebugScriptErrorCode = 'INVALID_SCRIPT' | 'CONVERSION_MISMATCH'

export class DebugScriptError extends Error {
  readonly code: DebugScriptErrorCode
  readonly line: number | undefined
  readonly path: string | undefined

  constructor(code: DebugScriptErrorCode, message: string, details: { line?: number; path?: string } = {}) {
    super(message)
    this.name = 'DebugScriptError'
    this.code = code
...
export class DshJsonlConversionError extends DebugScriptError {
  override readonly line: number

  constructor(line: number, message: string, code: DebugScriptErrorCode = 'INVALID_SCRIPT', path?: string) {
...
export function parseCanonicalScript(input: string, path?: string): DebugScript {
  let value: unknown
  try {
    value = JSON.parse(input.replace(/^\uFEFF/, '')) as unknown
  } catch {
    throw new DebugScriptError('INVALID_SCRIPT', `${path ?? 'debug script'} is not valid JSON`, path === undefined ? {} : { path })
  }
  return validateDebugScript(value, path)
}

/** Validate every canonical field and return a detached, JSON-only script. */
export function validateDebugScript(value: unknown, path?: string): DebugScript {
  const root = asPlainObject(value, 'script', path)
  assertExactKeys(root, ['type', 'version', 'steps'], 'script', path)
  if (root.type !== 'dsh-debug-script') invalid('type must be "dsh-debug-script"', path)
  ...
  return { type: 'dsh-debug-script', version: 1, steps }
}

/** Compile implicit and explicit inter-step waits without changing membership. */
export function compileDebugScript(value: DebugScript, overwriteWaitTimeMs?: number): CompiledDebugScript {
  const script = validateDebugScript(value)
  ...
  const executable: CompiledDebugStep[] = []
...
export function serializeDebugScript(value: DebugScript): string {
  return JSON.stringify(validateDebugScript(value))
}

/**
 * Convert a native DSH JSONL log into canonical debug-script JSON. Results and
 * approvals are evidence only; no historical result is put into the script.
 */
export function convertDshJsonl(input: string, path?: string): DebugScript {
  ...
  return validateDebugScript({ type: 'dsh-debug-script', version: 1, steps }, path)
}
...
function validateToolStep(value: JsonObject, stepPath: string, path?: string): DebugToolStep {
  if (typeof value.tool !== 'string' || !isDebugToolName(value.tool)) invalid(`${stepPath}.tool is not a valid tool name`, path)
...
function normalizeCall(name: string, rawArguments: string, line: number, path?: string): DebugToolStep {
  if (!isDebugToolName(name)) throw conversion(line, `invalid tool name ${JSON.stringify(name)}`, path)
  let args: unknown
  try {
...
function isWaitStep(step: DebugScriptStep): step is DebugWaitStep {
  return 'wait' in step
}

function invalid(message: string, path?: string): never {
  throw new DebugScriptError('INVALID_SCRIPT', path === undefined ? message : `${path}: ${message}`, path === undefined ? {} : { path })
}
