...
describe('ExecSessionService', () => {
  it('keeps a live session for polling and serializes writes', async () => {
    ...
    const first = service.write({ owner, jobId: 'codex-terminal-1', chars: 'one', yieldTimeMs: 0, signal: new AbortController().signal })
    const second = service.write({ owner, jobId: 'codex-terminal-1', chars: 'two', yieldTimeMs: 0, signal: new AbortController().signal })
    await expect(Promise.all([first, second])).resolves.toEqual([
      expect.objectContaining({ job_id: 'codex-terminal-1' }),
      expect.objectContaining({ job_id: 'codex-terminal-1' }),
    ])
  ...
  it('rejects a different owner and removes completed sessions', async () => {
    ...
    await expect(service.write({ owner: other, jobId: 'codex-terminal-1', chars: '', signal: new AbortController().signal }))
