...
describe('session output and retention regressions', () => {
  it('returns output that arrived between exec_command and the next empty write_stdin poll', async () => {
    ...
    expect(started.job_id).toBe('codex-terminal-1')

    backend.emit('stdout', 'output-after-exec')
    const result = await service.write({ owner, jobId: 'codex-terminal-1', chars: '', yieldTimeMs: 0, signal: signal() })
  ...
  it('returns already-unread output without waiting the default poll delay', async () => {
    ...
    const poll = service.write({ owner, jobId: 'codex-terminal-1', chars: '', signal: signal() })
  ...
  it('returns output beyond max_output_tokens across subsequent polls', async () => {
    ...
    await delay(0)
    expect((await service.write({ owner, jobId: 'codex-terminal-1', chars: '', yieldTimeMs: 0, signal: signal() })).exit_code).toBe(0)
  })
  ...
  it('keeps a completed session alive while token-capped terminal output has unread remainder', async () => {
    ...
    expect(first.job_id).toBe('codex-terminal-1')
  ...
  it('does not delete a naturally exited session after a non-empty write', async () => {
    ...
    await expect(service.write({ owner, jobId: 'codex-terminal-1', chars: 'late-input', yieldTimeMs: 0, signal: signal() })).rejects.toThrow()
    const result = await service.write({ owner, jobId: 'codex-terminal-1', chars: '', yieldTimeMs: 0, signal: signal() })
  ...
  it('emits one compact natural-exit notification without consuming the session output', async () => {
    ...
    expect(notifications).toEqual([
      'exec job codex-terminal-1 exited with code 0. Call write_stdin with job_id="codex-terminal-1" and chars="" to collect the remaining output.',
    ])

    const result = await service.write({ owner: notifyingOwner, jobId: 'codex-terminal-1', chars: '', yieldTimeMs: 0, signal: signal() })
