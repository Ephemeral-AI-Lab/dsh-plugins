...
describe('complex exec_command and write_stdin interactions', () => {
  ...
  it('completes one interactive session across three ordered calls', async () => {
    ...
    expect(partial.value?.job_id).toBe(jobId)
  ...
  it('interleaves partial input on two sessions before finishing either one', async () => {
    ...
    const firstId = first.value!.job_id!
    const secondId = second.value!.job_id!
    const firstPartial = await callTool(harness.writeStdin, { job_id: firstId, chars: 'PA', yield_time_ms: 0 }, execution(owner))
    const secondPartial = await callTool(harness.writeStdin, { job_id: secondId, chars: 'PA', yield_time_ms: 0 }, execution(owner))
    const firstDone = await callTool(harness.writeStdin, { job_id: firstId, chars: 'SS\n', yield_time_ms: 1_000 }, execution(owner))
    const secondDone = await callTool(harness.writeStdin, { job_id: secondId, chars: 'SS\n', yield_time_ms: 1_000 }, execution(owner))

    expect(firstPartial.value?.job_id).toBe(firstId)
    expect(secondPartial.value?.job_id).toBe(secondId)
  ...
  it('serializes concurrent writes that split one command across a queue', async () => {
    ...
    const [partial, completed] = await Promise.all([
      callTool(harness.writeStdin, { job_id: jobId, chars: 'PA', yield_time_ms: 0 }, execution(owner)),
      callTool(harness.writeStdin, { job_id: jobId, chars: 'SS\n', yield_time_ms: 1_000 }, execution(owner)),
    ])
  ...
  it('runs concurrent queued writes on separate sessions without cross-delivery', async () => {
    ...
    const [firstPartial, secondPartial] = await Promise.all([
      callTool(harness.writeStdin, { job_id: firstId, chars: 'PA', yield_time_ms: 0 }, execution(owner)),
      callTool(harness.writeStdin, { job_id: secondId, chars: 'PA', yield_time_ms: 0 }, execution(owner)),
    ])
    const [firstDone, secondDone] = await Promise.all([
      callTool(harness.writeStdin, { job_id: firstId, chars: 'SS\n', yield_time_ms: 1_000 }, execution(owner)),
      callTool(harness.writeStdin, { job_id: secondId, chars: 'SS\n', yield_time_ms: 1_000 }, execution(owner)),
    ])
  ...
  it('rejects a foreign write, then lets the original owner finish the session', async () => {
    ...
    const jobId = started.value!.job_id!
    const foreign = await callTool(harness.writeStdin, { job_id: jobId, chars: 'PASS\n', yield_time_ms: 1_000 }, execution(other))
    const partial = await callTool(harness.writeStdin, { job_id: jobId, chars: 'PA', yield_time_ms: 0 }, execution(owner))
    const completed = await callTool(harness.writeStdin, { job_id: jobId, chars: 'SS\n', yield_time_ms: 1_000 }, execution(owner))
  ...
  it('keeps a session owned after a foreign poll and accepts the owner write', async () => {
    ...
    const jobId = started.value!.job_id!
    const foreignPoll = await callTool(harness.writeStdin, { job_id: jobId, chars: '', yield_time_ms: 0 }, execution(other))
    const completed = await callTool(harness.writeStdin, { job_id: jobId, chars: 'PASS\n', yield_time_ms: 1_000 }, execution(owner))
  ...
  it('cleans one owner while another owner completes its independent session', async () => {
    ...
    const ownedId = owned.value!.job_id!
    const survivingId = surviving.value!.job_id!

    await owner.cleanup!()
    expect(harness.service.liveSessionCount).toBe(1)

    const survivingDone = await callTool(harness.writeStdin, { job_id: survivingId, chars: 'PASS\n', yield_time_ms: 1_000 }, execution(other))
    const cleanedWrite = await callTool(harness.writeStdin, { job_id: ownedId, chars: '', yield_time_ms: 0 }, execution(owner))
  ...
  it('races owner cleanup with delayed polls and preserves the other owner session', async () => {
    ...
    const ownedId = owned.value!.job_id!
    const survivingId = surviving.value!.job_id!
    const ownedPoll = callTool(harness.writeStdin, { job_id: ownedId, chars: '', yield_time_ms: 100 }, execution(owner))
    const survivingPoll = callTool(harness.writeStdin, { job_id: survivingId, chars: '', yield_time_ms: 100 }, execution(other))
    ...
    expect(survivingResult.value?.job_id).toBe(survivingId)
  ...
  it('uses a zero-wait poll before collecting delayed output from a slow command', async () => {
    ...
    const jobId = started.value!.job_id!
    const immediate = await callTool(harness.writeStdin, { job_id: jobId, chars: '', yield_time_ms: 0 }, execution(owner))
    const delayed = await callTool(harness.writeStdin, { job_id: jobId, chars: '', yield_time_ms: 1_000 }, execution(owner))

    expect(immediate.value?.job_id).toBe(jobId)
  ...
  it('polls between partial input and the final chunk without losing ownership or output', async () => {
    ...
    const jobId = started.value!.job_id!
    const partial = await callTool(harness.writeStdin, { job_id: jobId, chars: 'PA', yield_time_ms: 0 }, execution(owner))
    const poll = await callTool(harness.writeStdin, { job_id: jobId, chars: '', yield_time_ms: 25 }, execution(owner))
    const completed = await callTool(harness.writeStdin, { job_id: jobId, chars: 'SS\n', yield_time_ms: 1_000 }, execution(owner))

    expect(partial.value?.job_id).toBe(jobId)
    expect(poll.value?.job_id).toBe(jobId)
  ...
  it('returns a nonzero exit after a failed interactive command is sent in chunks', async () => {
    ...
    const jobId = started.value!.job_id!
    const partial = await callTool(harness.writeStdin, { job_id: jobId, chars: 'FA', yield_time_ms: 0 }, execution(owner))
    const failed = await callTool(harness.writeStdin, { job_id: jobId, chars: 'IL\n', yield_time_ms: 1_000 }, execution(owner))
  ...
  it('removes a failed session before a new session starts and keeps terminal polling idempotent', async () => {
    const failed = await callTool(harness.execCommand, { cmd: 'interactive:stale-failure', yield_time_ms: 0 }, execution(owner))
    const failedId = failed.value!.job_id!
    const failureResult = await callTool(harness.writeStdin, { job_id: failedId, chars: 'FAIL\n', yield_time_ms: 1_000 }, execution(owner))
    ...
    const recoveredResult = await callTool(harness.writeStdin, {
      job_id: recovered.value!.job_id!,
      chars: 'PASS\n',
      yield_time_ms: 1_000,
    }, execution(owner))
    const stalePoll = await callTool(harness.writeStdin, { job_id: failedId, chars: '', yield_time_ms: 0 }, execution(owner))
    ...
    expect(stalePoll.isError).toBe(false)
    expect(stalePoll.content[0]?.text).toContain('no unread output remains')
  })
  ...
  it('does not let a pre-aborted write destroy a still-live session', async () => {
    ...
    controller.abort()
    const abortedWrite = await callTool(harness.writeStdin, { job_id: jobId, chars: 'PASS\n', yield_time_ms: 1_000 }, { agent: owner, signal: controller.signal })
    expect(harness.service.liveSessionCount).toBe(1)
    const completed = await callTool(harness.writeStdin, { job_id: jobId, chars: 'PASS\n', yield_time_ms: 1_000 }, execution(owner))
  ...
  it('aborts a delayed poll and removes the session before a stale follow-up', async () => {
    ...
    const foreground = await callTool(harness.execCommand, { cmd: 'foreground', yield_time_ms: 1_000 }, execution(owner))
    const controller = new AbortController()
    const pendingPoll = callTool(harness.writeStdin, { job_id: jobId, chars: '', yield_time_ms: 1_000 }, { agent: owner, signal: controller.signal })

    await new Promise<void>(resolve => setTimeout(resolve, 10))
    controller.abort()
    const abortedPoll = await pendingPoll
    const stalePoll = await callTool(harness.writeStdin, { job_id: jobId, chars: '', yield_time_ms: 0 }, execution(owner))
  ...
  it('aborts the first queued operation and rejects a completion queued behind it', async () => {
    ...
    await new Promise<void>(resolve => setTimeout(resolve, 10))
    const queuedCompletion = callTool(harness.writeStdin, { job_id: jobId, chars: 'PASS\n', yield_time_ms: 1_000 }, execution(owner))
    controller.abort()
  ...
  it('disposes multiple live sessions while their polls are waiting', async () => {
    ...
    const firstId = first.value!.job_id!
    const secondId = second.value!.job_id!
    const firstPoll = callTool(harness.writeStdin, { job_id: firstId, chars: '', yield_time_ms: 1_000 }, execution(owner))
    const secondPoll = callTool(harness.writeStdin, { job_id: secondId, chars: '', yield_time_ms: 1_000 }, execution(owner))

    await new Promise<void>(resolve => setTimeout(resolve, 10))
    await harness.service.dispose()
    const [firstResult, secondResult] = await Promise.all([firstPoll, secondPoll])
    const staleFirst = await callTool(harness.writeStdin, { job_id: firstId, chars: '', yield_time_ms: 0 }, execution(owner))
  ...
  it('keeps concurrent interactive success and failure results tied to their owners', async () => {
    ...
    const successId = success.value!.job_id!
    const failureId = failure.value!.job_id!
    const [successResult, failureResult] = await Promise.all([
      callTool(harness.writeStdin, { job_id: successId, chars: 'PASS\n', yield_time_ms: 1_000 }, execution(owner)),
      callTool(harness.writeStdin, { job_id: failureId, chars: 'FAIL\n', yield_time_ms: 1_000 }, execution(other)),
    ])
  ...
  it('makes repeated owner cleanup calls idempotent across partially written sessions', async () => {
    ...
    const firstAfterCleanup = await callTool(harness.writeStdin, { job_id: firstId, chars: 'SS\n', yield_time_ms: 0 }, execution(owner))
    const secondAfterCleanup = await callTool(harness.writeStdin, { job_id: secondId, chars: 'SS\n', yield_time_ms: 0 }, execution(owner))
