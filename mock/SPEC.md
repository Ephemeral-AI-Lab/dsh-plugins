# DSH mock implementation specification
...
purpose is deterministic execution of one mock command or a replay script
...
`mock` and the model id `mock`:

```text
per-turn /mock command
        |
        v
mock command/replay controller
        |
        v
public DSH per-turn model selection: { provider: "mock", model: "mock" }
        |
        v
MockAdapter (public @deepseek-ai/dsh-llm LlmAdapter)
        |
        v
real DSH AgentLoop
        |
...
model page MUST NOT be required to select `mock/mock`, and the plugin
MUST NOT permanently change an agent's configured provider or model. A command
selects the mock route for that turn only. The route may be discoverable to
...
MUST unregister `mock`, abort or release adapter-owned state, remove
...
The adapter MUST expose provider metadata for `mock` and model metadata
for `mock`, for example:

```text
providerInfo("mock") -> { id: "mock", name: "Mock" }
resolve/list model("mock", "mock")
  -> { provider: "mock", id: "mock", name: "Deterministic Mock Model" }
```
...
The plugin MUST register `/mock` through the public DSH slash-command/message
command surface. A slash command received as a normal message, followup, or
steer command MUST be parsed the same way and scheduled as the next mock
turn. Followup and steer do not create a special execution mode.

Every accepted mock command creates a turn-local plan keyed by the current
DSH `sessionId` and a unique mock turn id. The command dispatcher MUST hand
that plan to the normal AgentLoop through the public per-turn route override:

```text
{ provider: "mock", model: "mock", mockTurnId: "..." }
```

No global `mockEnabled` flag may affect later turns. After a mock turn is
complete, a non-slash message MUST use the real provider configured for that
turn. Mock and real-provider turns MAY coexist in one session transcript and
MUST remain distinguishable by their per-turn route and durable events.

### 3.1 `/mock run`

`/mock run` MUST accept exactly one executable unit: either one tool call or
one parallel group. Tool arguments MUST be JSON objects. The command grammar
is:

```text
/mock run tool_name(JSON_OBJECT)
/mock run [tool_name(JSON_OBJECT) tool_name(JSON_OBJECT) ...]
```
...
/mock run exec_command({"cmd":"Get-Location"})
/mock run [probe_a({"value":"a"}) probe_b({})]
```
...
`/mock run` MUST NOT accept wait syntax. In particular, `--wait`, `wait`, a
...
```text
/mock replay <path>
/mock replay <path> --overwrite-wait-time-ms <N>
```
...
detector. v1 MUST support canonical mock-script JSON and DSH JSONL. Additional
...
## 4. Canonical mock-script format
...
```text
Script       = { type: "dsh-mock-script", version: 1, steps: Step[] }
...
`dsh-mock-script` JSON plus source locations for diagnostics. It MUST NOT
...
Only the final response ends the mock turn. Intermediate responses MUST end
...
the mock plan MUST enter a terminal failed state and no later top-level step
...
mock command, the agent becomes `running` when the normal turn starts. It
...
All mutable plan state MUST be keyed by the DSH `sessionId` and a mock turn
...
DSH session event/persistence API, with at least the session id, mock turn id,
...
normal DSH events and report that optional mock lifecycle persistence is
...
| runtime/policy/output error | Normal DSH tool failure or denial | Preserve normal event/error; stop the mock plan |
...
indicator above the composer while the current mock turn is running. It MUST
...
│   ├── commands.ts              # /mock run and /mock replay parsing/routing
...
│   ├── events.ts                # public durable mock lifecycle events
...
- single `/mock run` accepts an object and rejects malformed/non-JSON args;
- one bracketed `/mock run [tool(... ) tool(...)]` group is accepted;
- waits and multiple sequential calls are rejected by `/mock run`;
...
- `mock` and `mock` resolve, then disappear on plugin disposal;
- the persistent model-page selection is unchanged;
- slash mock, followup/steer mock, and real-provider turns coexist in one
  session;
...
does not expose model-page mock selection, does not accept waits in `/mock
...
tool or one parallel group for `/mock run`; multi-step behavior belongs to
