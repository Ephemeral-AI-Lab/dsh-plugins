...
- The mock path uses the real `AgentLoop` and `ToolRuntime`; the adapter emits
...
memory. The next ordinary user turn may then be routed to `mock`.
...
Acceptance criteria: zero plans, timers, listeners, and mock provider routes
...
- Return a structured `MOCK_PROTOCOL` failure with useful IDs and session
  context.
...
- Append a `mock/status` failure event for errors before the replay starts.
...
fields, such as reasoning configuration, may be lost or inherit mock values.

Required work:

- Snapshot the complete `LlmCallConfig` before entering mock mode.
- Restore the complete snapshot on success, error, cancellation, and disposal.
- Keep mock mode slash-command-driven; do not expose a deterministic mock
  model selector.
...
- Assert no explicit deterministic mock model appears in the model catalog.
...
- Verify they cannot block or advance the mock replay.
...
not have an installed `dsh-mock` package.
...
- `/mock` slash-command registration.
- No explicit deterministic mock model selector.
