...
Bounded read-only review of DSH session/JSONL fixtures for the `mock`
...
session JSONL and should not be mistaken for canonical mock scripts.
...
3. **Override-file detection:** `replay.override.json` is an array of
   ...
   `dsh-mock-script` JSON. The detector must select a separate documented
