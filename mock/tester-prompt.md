# Mock Agent Test-Phase Prompt

You are the test owner for the `dsh-mock` plugin.
...
- Mock mode is entered only through `/mock`; there must be no explicit
  deterministic mock model selector.
...
Start the host web app with the mock plugin patch and use a fresh browser
...
In a new session, type `/mock` and verify the command is offered and can be
submitted.

Expected:

- `/mock run <tool>(<JSON>)` and `/mock replay <path>` are available.
- The command queues a normal user message into AgentLoop.
- No mock model appears in the model-selection UI.
...
- The mock route is not left active afterward.
...
- The canonical shape is accepted:
  `type: dsh-mock-script`, `version: 1`, `steps`.
...
```text
/mock replay "<extracted-session.jsonl>" --overwrite-wait-time-ms 0
```
...
```text
/mock replay "C:\path\to\session.zip"
```
...
- Each invalid result terminates deterministically with `MOCK_PROTOCOL`.
...
After successful, failed, and cancelled mock runs, send an ordinary user
message.
...
- The ordinary turn uses the original configured provider/model and route
  settings.
- No deterministic mock model is exposed in model selection.
...
4. session ID and mock run ID;
5. browser screenshots or visible status text for progress/failure cases;
6. durable `tool/call`, `tool/result`, and `mock/status` event excerpts;
...
- the browser cannot load `/mock` from a clean package-name profile.
