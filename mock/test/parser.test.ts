...
import { MockCommandParseError, parseToolCall } from '../src/parser.js'

describe('mock command parser', () => {
  it('parses an explicit empty JSON object', () => {
    expect(parseToolCall('tool_a({})')).toEqual({ name: 'tool_a', arguments: {} })
    expect(() => parseToolCall(' tool_a() ')).toThrow(MockCommandParseError)
  })
  ...
  it.each([
    ...
    expect(() => parseToolCall(input)).toThrow(MockCommandParseError)
  })
