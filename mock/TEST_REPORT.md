...
The requested host mock E2E target is not present in the host checkout:

```text
apps/web/tests/mock.e2e.ts  -> False
apps/web/tests/mock.web.ts  -> False
```
...
```text
Mock run/replay completed (N executable step[s]).
```
...
`ui/ui-ux.md`: no invented result transcript and no second large mock panel.
...
5. Added deterministic `MOCK_PROTOCOL` handling for wrong, duplicate,
   unknown, stale, mismatched, and cross-session result correlations.
6. Restored the complete real LLM route configuration after mock turns,
...
7. Kept auxiliary calls on the real host route so title/compaction-style calls
   cannot consume a pending mock tool result.
...
| `/mock run exec_command({"cmd":"Write-Output UI_FINAL_COMPACT"})` | PASS; 1/1, compact output |
| `/mock replay <extracted session.jsonl> --overwrite-wait-time-ms 0` | PASS; 7/7 |
...
The first command did not select a mock file because that file does not
...
filter: apps/web/tests/mock.e2e.ts
```

The `90 skipped` and `33 skipped` counters belong to that broad host-suite
run, not to mock: `90` is skipped test cases, while `33` is skipped
...
the LSP E2E file with four skipped cases. The missing mock file itself
contributed no passing or skipped mock tests.
